...
adresar = "C:/Správa exekucí/"
try:
    chdir(adresar)
except FileNotFoundError:
    mkdir(adresar)
    with open(adresar + "velikost_textu.txt", mode="w", encoding="utf-8") as struktura:
        print(8, file=struktura)
...
class exekuce:
    ...
    def __init__(self):
        self.zaznamy = []
        self.nazev_soubor = "C:/Správa exekucí/exekuce.csv"
        self.cesta_k_textu = "C:/Správa exekucí/velikost_textu.txt"
        self.cesta_do_Excelu = "C:/Správa exekucí/sestava_pro_Excel.csv"
    ...
    def kontrola_RSD(self, RC, Datum_prijeti, Nabyti_pravni_moci, RSD, cislo_exekuce, prednostni, celkovy_dluh, cislo_uctu, datovka, email, doplatit, poznamka):
        seznam_cisel = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]
        if RSD[1] not in seznam_cisel:
            RSD = "0" + RSD

        if RSD[4] not in seznam_cisel:
            RSD = RSD[:3] + "0" + RSD[3:]
        RSD = RSD.replace(".", "/")
        self.pridej_zaznam(RC, Datum_prijeti, Nabyti_pravni_moci, RSD, cislo_exekuce, prednostni, celkovy_dluh, cislo_uctu, datovka, email, doplatit, poznamka)


    def serad(self):
        tabulka = pd.DataFrame(self.nalezeno, columns =["RC", "Datum_prijeti", "Nabyti_pravni_moci", "RSD", "cislo_exekuce", "prednostni", "celkovy_dluh", "cislo_uctu", "datovka", "email", "doplatit", "poznamka"]) 
        tabulka["RSD"] = pd.to_datetime(tabulka["RSD"])  #timhle se to prevede na format, ktery je pandas schopny seradit
        serazeno = tabulka.sort_values(by='RSD')    # seradi se
        serazeno["RSD"] = serazeno["RSD"].dt.strftime("%m/%d/%Y")   # zformatuje se, aby odpovidal format data
        self.nalezeno = serazeno.values.tolist()
    ...
    def do_Excelu(self, cesta_do_Excelu):
        if self.zaznamy == []:
            tk.messagebox.showwarning("Error", "Nejsou žádná data k exportu\n nebo nemáš načtené exekuce.")
        else:
            with open(self.cesta_do_Excelu, "w", encoding="utf-8") as excel:
    ...
    def pridej_zaznam(self, RC, Datum_prijeti, Nabyti_pravni_moci, RSD, cislo_exekuce, prednostni, celkovy_dluh, cislo_uctu, datovka, email, doplatit, poznamka):
        ...
        if poznamka == "":
            poznamka = " "
        RSD = RSD.replace(".", "/")
        Datum_prijeti = Datum_prijeti.replace(".", "/")
        Nabyti_pravni_moci = Nabyti_pravni_moci.replace(".", "/")
...
class exekuceGUI(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # sestava pro Excel
        self.frame_excel = tk.Frame()
        self.frame_excel.pack()

        self.button_excel = tk.Button(self.frame_excel, width=70, text="Vytvoř sestavu pro Excel", command=self.sestava_Excel)
        self.button_excel.pack(side=LEFT)
        ...
        self.entry_RC.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_Datum_prijeti.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_cislo_exekuce.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_prednostni.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_celkovy_dluh.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_cislo_uctu.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_doplatit.config(state=NORMAL, justify=CENTER)
        ...
        self.entry_poznamka.config(state=NORMAL, justify=CENTER)
    ...
    def sestava_Excel(self):
        self.exekuce.do_Excelu(self)
    ...
    def vyhledej(self):
        ...
        """
        Tady musí být volání funkce self.exekuce.seradit(self.exekuce.nalezeno), která převezme data z proměnné self.nalezeno
        setřídí nalezená data podle datumu ve sloupci ŘSD od nejstaršího
        a přepíše původní proměnnou seřazeným seznamem.
        """
    ...
    def on_pridej(self):
        ...
        self.nacti()          
        self.exekuce.kontrola_RSD(self.RC.get(),
